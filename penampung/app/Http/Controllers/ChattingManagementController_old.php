...
class ChattingManagementController extends Controller
{
    ...
    public function unreadConversation(Request $request)
    {
        ...
        $percakapan = Percakapan::with(['latestMessage', 'customer', 'branch'])
            ->whereHas('message', function ($query) {
                $query->where('is_read', 'FALSE');
            })
            ->where('kd_cabang', $request->cabangId)
            ->count();
    ...
    {
        $message = Message::where('conversation_id', $request->conversationId)->where('is_read', 'FALSE')->count();
    ...
    {
        $percakapan = Percakapan::where('id', $request->conversation_id)->first()->update(['receive_id' => null]);
        return response()->json($percakapan);
    }
}