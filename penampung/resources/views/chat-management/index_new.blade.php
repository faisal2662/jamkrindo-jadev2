                            ...
                            <div class="chat-header clearfix">
                                <div class="row">
                                    ...
                                    <div class="col-lg-6 hidden-sm ">
                                        <div class="float-end">
                                            <button id="close-chatting"
                                                class="btn btn-outline-secondary btn-sm close-chat">Close
                                                Chat</button>
    ...
    <script>
        ...
        function loadConversations() {
            $.get('conversations', function(data) {
                ...
                data.forEach(conversation => {
                    ...
                    if (today == nowDate()) {
                        today = "Hari ini";
                    } else if (tanggal == Tanggal(nowDate(), 1)) {
                        today = "Kemarin";
                    }
                    ...
                    if (conversation.customer.foto_customer) {
                        ...
                        $.get('chat/unread-message', {
                            ...
                            //     `  <li class="clearfix list-message" id="tes" data-name="${conversation.customer.nama_customer}" data-img="${conversation.customer.foto_customer}" data-id="${conversation.id}" data-cabang="${conversation.branch.nm_cabang}">                            
                        // <div id="is_read_message">   </div>    
                        //     <img src="{{ asset('assets/img/customer/${conversation.customer.foto_customer}') }}" alt="avatar">
                        //         <div class="about">
                        //             <div class="name">${conversation.customer.nama_customer}</div>
                        //             <div class="status">  <span class="time-last"> ${waktu(conversation.latest_message.created_date)}</span></div>
                        //         </div>
                        //         <div class="receive"> Penerima : <span>Effendy</span> </div>
                        //     </li>`);
                            if (data > 0) {
        ...
        function connectWebSocket(chatId = null) {
            return new Promise((resolve, reject) => {
                ...
                const wsUrl = `${baseWsUrl}id_cabang=${cabangId}&id_user={{ auth()->user()->kd_user }}&conversation_id=${chatId}`;
                ...
                socket.onmessage = function(event) {
                    ...
                    if (data.conversation_id == inputConversationId) {
                        ...
                        $.post("{{ route('chat-read') }}", {
                            ...
                            loadMessages(inputConversationId)
                            $('#box-message').append(
                                ` <li class="clearfix">
                ...
                //           </div>`);
                    ...
                    $(`#conversationList li[data-id="${data.conversation_id}"] .time-last`).text(data
                        .created_at);
                ...
                data.forEach(message => {
                    ...
                    //       </li> `);
                        ...
                        <div class="message other-message float-end">
                             <div class=""><img src="{{ asset('assets/files/${message.message}') }}" class="img-thumbnail" max-width="50px;"  ></div><button onclick="downloadBtn('${message.message}')"  style="border:none" id="downloadBtn" class="badge bg-secondary downloadBtn">Unduh File</button>
...
`);
                        ...
                        //         ${waktu(message.created_date)}  <i class="${is_read}"></i></span></div> ` );
                            $('#box-message').append(`
                         <li class="clearfix">
                             <div class="message other-message float-end" >  <div class=""><video style="max-width: 100%; height: auto;" controls ><source src="{{ asset('assets/files/${message.message}') }}"  type="video/mp4"></video></div><button onclick="downloadBtn('${message.message}')"  style="border:none" id="downloadBtn" class="badge bg-secondary downloadBtn">Unduh File</button> <div class="message-data text-end" >
                                <span class="message-data-time" >${waktu(message.created_date)}  ${is_read}</span>
                            </div></div>
                    ...
                    //         </div> </li>`);
                        ...
                        //         ${waktu(message.created_date)}  <i class="${is_read}"></i></span></div> `);
                            ...
                            $('#box-message').append(` <li class="clearfix">
                                <div class="message my-message">    <div class=""><img src="{{ asset('assets/files/${message.message}') }}" class="img-thumbnail" max-width="50px;"  ></div><button onclick="downloadBtn('${message.message}')"  style="border:none" id="downloadBtn" class="badge bg-secondary downloadBtn">Unduh File</button><div class="message-data text-end">
                                    <span class="message-data-time">${waktu(message.created_date)}  </span>
                                </div></div>
                                 </li>`);
                        ...
                        //         ${waktu(message.created_date)}  <i class="${is_read}"></i></span></div> ` );
                            ...
                            $('#box-message').append(` <li class="clearfix">
                                <div class="message my-message">     <div class=""><video style="max-width: 100%; height: auto;" controls ><source src="{{ asset('assets/files/${message.message}') }}"  type="video/mp4"></video></div><button onclick="downloadBtn('${message.message}')"  style="border:none" id="downloadBtn" class="badge bg-secondary downloadBtn">Unduh File</button><div class="message-data text-end">
                                    <span class="message-data-time">${waktu(message.created_date)}   </span>
                                </div> </div>
                                </li>`);
                            //       $('#box-message').append( `<div class="  mb-2 rounded-3 justify-content-end" id="penerima" style="padding:5px;background-color: #457B9D;margin-right: 40px;margin-left: 10px;">
        ...
        $('#close-chatting').click(function() {
            ...
            // formData.append('conversationId', 7);
            $.ajax({
                ...
                success: function(res) {
                    ...
                    connectWebSocket(conversation).then(() => {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    //                                         </li>`
                        ...
                        <li class="clearfix">
                        <div class="message other-message float-end">
                             <div class=""><img src="assets/files/${data.message}" class="img-thumbnail" max-width="50px;"  ></div><button onclick="downloadBtn('${data.message}')"  style="border:none" id="downloadBtn" class="badge bg-secondary downloadBtn">Unduh File</button>
                            <div class="message-data text-end">
                                <span class="message-data-time">${nowTime()}  <i class="bi bi-eye-slash"></i></span>
                            </div>
                                </div>
                        ...
                        //         ${waktu(message.created_date)}  <i class="bi bi-eye"></i></span></div> ` );
                            $('#box-message').append(`
                         <li class="clearfix">
                             <div class="message other-message float-end" >  <div class=""><video style="max-width: 100%; height: auto;" controls ><source src="assets/files/${data.message}"  type="video/mp4"></video></div><button onclick="downloadBtn('${data.message}')"  style="border:none" id="downloadBtn" class="badge bg-secondary downloadBtn">Unduh File</button> <div class="message-data text-end" >
                                <span class="message-data-time" >${nowTime()}  <i class="bi bi-eye-slash"></i></span>
                            </div></div>
        ...
        $(document).ready(function() {
            ...
            $(document).on('click', '.list-message', function() {
                ...
                if (id_user != $(this).find('span.adminReceive').data('id')) {
                    if (!$(this).find('span.adminReceive').data('id')) {
                ...
                $(this).find('#is_read_message').empty();
                // console.log($('#close-chatting'))
                $('button.close-chat').attr('data-conversation', currentConversationId);
