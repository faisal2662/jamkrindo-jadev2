...
@section('main')
<div class="pagetitle">
    <h1>Laporan DWH SPD</h1>
    <nav>
        <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Home</a></li>
            <li class="breadcrumb-item active">Laporan DWH SPD</li>
        </ol>
    </nav>
</div><!-- End Page Title -->
...
<section class="section">
    <div class="row mb-3">
        <div class="col-md-3">
            <button class="btn btn-warning btn-sm" id="generateDWH">Generate DWH Manual</button>
        </div>
    </div>
    <div class="row">
        <div class="col-lg-12">
                ...
                <div class="card-body">
                    ...
                    @if (Session::has('success'))
                        ...
                        <div class="table-responsive">
                            <table class="table datatable table-hover table-striped display expandable-table" id="customers-table">
                                <thead>
                                    <tr>
                                        <th style="font-size: 12px">No.</th>
                                        <th style="font-size: 12px">No Surat</th>
                                        <th style="font-size: 12px">No SP</th>
                                        <th style="font-size: 12px">Tanggal SP</th>
                                        <th style="font-size: 12px">Nasabah</th>
                                        <th style="font-size: 12px">Kantor Wilayah</th>
                                        <th style="font-size: 12px">Wilayah Kerja</th>
                                        <th style="font-size: 12px">Penerima Jaminan</th>
                                        <th style="font-size: 12px">lob</th>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#btnFilter').on('click', function(e) {
            e.preventDefault();
            table.ajax.url("{{ route('report.dwh.getData') }}" + '?startDate=' + $('#startDate').val() + '&endDate=' + $('#endDate').val()).load();
            $('#btnPdf').removeAttr('disabled');
            $('#btnExcel').removeAttr('disabled');
            $('#btnPreview').removeAttr('disabled');
        });
        $('#generateDWH').on('click', function(e) {
            e.preventDefault();
            $.ajax({
                url: "/jamkrindo/get-dwh",
                type: "GET",
                success: function(response){
                    alert(response.message);
                },
                error: function(xhr){
                    alert('Generate DWH Gagal');
                }
            });
            // table.ajax.url("{{ route('report.dwh.getData') }}" + '?startDate=' + $('#startDate').val() + '&endDate=' + $('#endDate').val()).load();
        });
        $('#btnPdf').on('click', function(e) {
            e.preventDefault();
            window.open('{{ route("report.dwh.raw.pdf") }}' + '?startDate=' + $('#startDate').val() + '&endDate=' + $('#endDate').val(), '_blank');
        });
        $('#btnExcel').on('click', function(e) {
            e.preventDefault();
            window.open('{{ route("report.dwh.raw.excel") }}' + '?startDate=' + $('#startDate').val() + '&endDate=' + $('#endDate').val(), '_blank');
        });
        $('#btnPreview').on('click', function(e) {
            e.preventDefault();
            window.open('{{ route("report.dwh.preview") }}' + '?startDate=' + $('#startDate').val() + '&endDate=' + $('#endDate').val(), '_blank');
        });
    });
</script>
@stop
