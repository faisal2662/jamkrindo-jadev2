                                    ...
                                    <td> 
                                        ...
                                        @if (!empty($item->detail))
                                            <div class="short-conversation" id="{{ $noc }}short-conversation_{{ $no }}">
                                                ...
                                                @for ($i = 0; $i < 5; $i++)
                                                    @if (isset($item->detail[$i]))
                                                    <div class="row border-bottom" style="margin: 0;padding:0;">
                                                        @if ($item->detail[$i]->status == '1')
                                                            <div class="col-12 text-start " style=" margin-right: 10px;">
                                                                <span class=" text-primary" style="display:block;">{{ $item->detail[$i]->employee_name }}</span>
                                                                <span style="display:block;"> {{ $item->detail[$i]->message }} </span>
                                                                <span style="display:block;">
                                                                    {{-- <small>{{ \Carbon\Carbon::parse($messages[$i]->created_date)->translatedFormat('j F Y h:i') }}</small> --}}
                                                                    <small>{{ $item->detail[$i]->created_date }}</small>
                                                                </span>
                                                            </div>
                                                        @elseif($item->detail[$i]->status == '0')
                                                            <div class="col-12  " style=" text-align:end; margin-right:5px; margin-left: 10px;">
                                                                <span class=" text-primary" style="display:block;">{{ $item->detail[$i]->nama_customer }}</span>
                                                                <span style="display:block;"> {{ $item->detail[$i]->message }} </span>
                                                                <span style="display:block;">
                                                                    {{-- <small>{{ \Carbon\Carbon::parse($messages[$i]->created_date)->translatedFormat('j F Y h:i') }}</small> --}}
                                                                    <small>{{ $item->detail[$i]->created_date }}</small>
